...
export function Hosts() {
  return (
    <section id="hosts" className="py-24 sm:py-32">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 grid gap-12 lg:grid-cols-2 lg:items-center">
        <Reveal>
          <h2 className="text-4xl sm:text-5xl font-bold tracking-tighter">
            Hire your hosts.
          </h2>
          <p className="mt-4 max-w-[48ch] text-lg text-text-2 leading-relaxed">
            One to three DJs. Name them, pick their voices from thousands on
            Fish Audio, and set their personality in a single line.
          </p>
          <p className="mt-4 max-w-[48ch] text-text-2 leading-relaxed">
          ...
          {HOSTS.map((h, i) => (
            <Reveal key={h.name} delay={i * 0.08}>
              <div className="glass flex items-center gap-4 p-4 sm:p-5">
                <span
                  className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-lg font-bold text-[#14060a]"
                ...
                <span className="ml-auto hidden sm:inline rounded-full border border-white/10 bg-white/4 px-3 py-1 text-xs text-text-2">
                  &ldquo;{h.style}&rdquo;
                </span>
              </div>
            </Reveal>
